...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    // Handle GET request to fetch courier service details based on the courierServiceId from the URL
    extract($_GET);
    $db = dbConn();
    $sql = "SELECT * FROM courier_service WHERE CourierServiceId='$courierProfileId'";
    $result = $db->query($sql);
    $row = $result->fetch_assoc();

    // Assign fetched category details to variables
    $cou_company_name = $row['CouCompanyName'];
    $cont_person_name = $row['ContPersonName'];
    $cou_contact_mobile = $row['ContactMobile'];
    $cou_alt_mobile = $row['AlternateMobile'];
    $email = $row['Email'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
     ...
     if (empty($message)) {
        $db = dbConn();
       $sql = "UPDATE courier_service SET CouCompanyName='$cou_company_name', ContPersonName='$cont_person_name',"
               . " Email='$email', ContactMobile='$cou_contact_mobile', AlternateMobile='$cou_alt_mobile', "
               ...
               . "BankName='$bank_name', Branch='$bank_branch' WHERE CourierServiceId='$CourierServiceId'";
    ...
    <div class="col-12">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Update Courier Service</h3>
            </div>         
            ...
            <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
                ...
                <div class="card-footer">
                  <!-- Hidden input field to pass the CourierServiceId with the form submission -->
                    <input type="hidden" name="CourierServiceId" value="<?= $CourierServiceId ?>">
