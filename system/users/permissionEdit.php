...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    // Handle GET request to fetch permission details based on the permissionid from the URL
    extract($_GET);
    $db = dbConn();
    $sql = "SELECT * FROM user_modules WHERE Id='$permissionid'";
    $result = $db->query($sql);
    $row = $result->fetch_assoc();

    // Assign fetched permission details to variables
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($message)) {
        ...
        if (empty($message)) {
            ...
            $sql = "UPDATE user_modules 
                    SET ModuleId='$module', RoleId='$role_id', `Add`='$add', `View`='$view', `Update`='$update', `Delete`='$delete'
                    WHERE Id='$Id'";
